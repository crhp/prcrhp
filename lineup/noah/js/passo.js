...
$(function() {
  ...
  var quizModal = function() {
    ...
    $('.check a').on('click', function() {
      ...
      $target.css({paddingTop: margin}).animate({opacity: 1}, duration);
    });

    $('.modal .close a, .modal .alpha').on('click', function() {
      $(this).parentsUntil('.item').filter('.modal').fadeOut(duration / 2);
    });
  ...
  var clickEvent = function() {
    ...
    var gaEvent = function(event, tag1, tag2, target) {
      ...
      var hrefTarget = $(event.currentTarget).attr('target');
      
      if (hrefTarget === '_blank') {
        _gaq.push(
          ['_trackEvent', 'prius', tag1, tag2]
        );
      } else {
        _gaq.push(
          ['_set', {hitCallback: function() { document.location.href = target.href; }}],
          ['_trackEvent', 'prius', tag1, tag2]
        );
    ...
    $('.gaEvent').on('click', function(event) {
      var s = $(this).attr('data-gaevent').split(',');
      gaEvent(event, s[0], s[1], this);
    });
